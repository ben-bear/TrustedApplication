...
import untils.AMapUtil;
import untils.ToastUtil;
...
//        mStartPoint = new LatLonPoint(intent.getFloatExtra("start_latitude",0f),
//                intent.getFloatExtra("start_longtitude",0f));
//
//        mEndPoint =  new LatLonPoint(intent.getFloatExtra("end_latitude",0f),
//                intent.getFloatExtra("end_longtitude",0f));
//        mStartPoint.setLatitude(intent.getDoubleExtra("start_latitude",0f));
//        mStartPoint.setLongitude(intent.getDoubleExtra("start_longtitude",0f));
//        mEndPoint.setLatitude(intent.getDoubleExtra("end_latitude",0f));
//        mEndPoint.setLongitude(intent.getDoubleExtra("end_longtitude",0f));
        start_latitude = intent.getDoubleExtra("start_latitude",0f);
        start_longtitude = intent.getDoubleExtra("start_longtitude",0f);
        /**
         * 初始化起点和终点
         */
        mStartPoint.setLatitude(intent.getDoubleExtra("start_latitude",0f));
        ...
        mStartPoint_bus.setLongitude(intent.getDoubleExtra("start_longtitude",0f));
        mStartPoint_bus.setLatitude(intent.getDoubleExtra("start_latitude",0f));
        mEndPoint_bus.setLongitude(intent.getDoubleExtra("end_longtitude",0f));
        mEndPoint_bus.setLatitude(intent.getDoubleExtra("end_latitude",0f));

//        LatLng myPosition = new LatLng(intent.getDoubleExtra("start_latitude",0f),
//                intent.getDoubleExtra("start_longtitude",0f));
//
//        changeCamera(CameraUpdateFactory.newCameraPosition(new CameraPosition(myPosition,18,0,0)));

        Toast.makeText(getApplicationContext(),""+intent.getDoubleExtra("end_latitude",0f),Toast.LENGTH_SHORT).show();


        init();
		setfromandtoMarker();

		onDriveClick(mapView);
	...
	private void init() {
		...
		//把焦点放在起点
        LatLng myPosition = new LatLng(start_latitude,start_longtitude);
        changeCamera(CameraUpdateFactory.newCameraPosition(new CameraPosition(myPosition,12,0,0)));
